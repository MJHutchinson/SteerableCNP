...
def main(args):
    ...
    equiv_cnp = LightningEquivCNP(**args.model)
    datamodule = LightningGPDataModule(
        dataset, batch_size=args.batch_size, splits=args.splits
    )

    log_dir = os.path.join(args.paths.logdir, dataset.name, equiv_cnp.name)
    ...
    #     CSVLogger(save_dir=run_dir, name=None, version=""),
    #     # Log data to tensorboard
    #     TensorBoardLogger(save_dir=run_dir, name=None, version=""),
    # ]
    ...
    #     InferencePlotCallback(n_plots=3, dirpath=run_dir + "/plots"),
    #     # Callback to plot comparison of the inference to the GP drawn from
    #     GPComparePlotCallback(
    #         n_plots=3,
    #         dirpath=run_dir + "/plots",
