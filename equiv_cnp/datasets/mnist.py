...
class MultiMNIST(data.Dataset):
    def __init__(
        ...
        data_path = os.path.join(
            self.dir,
            f"{'train' if train else 'test'}_seed_{self.seed}_digits_{self.n_digits}_rotate_{self.rotate}_translate_{self.translate}_size_{canvas_multiplier}.pt",
        )
    ...
    def make_multi_mnist(
        self, dataset, seed, rotate, translate, n_digits, base_size, canvas_size
    ):
